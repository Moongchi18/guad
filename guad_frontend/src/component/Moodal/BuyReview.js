...
function BuyReview({ modalOpen, closeModal, item }) {
  ...
  const handleSubmit = () => {    
    axios.post(`https://${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/review`, {itemNum : item.itemNum, sellerEmail : item.sellerEmail, writerNickname : nickname, contents, starPoint : rating})
    .then((response) => {

...
function Manager() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/admin/member`).then((response) => {
      console.log(response.data);
      setDatas(response.data);
    });

    axios
      .get(`${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/notify/admin/list`)
      .then((response) => {
              ...
              <div className={style.user_list} key={memberList.memberNum}>
                <img
                  src={
                    ...
                    memberList.loginImgName !== "null"  
                      ? `${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/image/member/${memberList.loginImgName}`
                      : logo_d
                  }
                  alt="1"
                ></img>ㅊ
            ...
            {datas2 &&
              datas2.map((notify) => (
                <li key={notify.notifyNum} className={style.notify_list}>
                  <img src={`${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/image/${notify.itemImgName}`} alt={"img" + notify.notifyNum}></img>
